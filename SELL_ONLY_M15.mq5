...
//|     Sell-only — M15 — Magic 203106 | Prefix GPS_M15_*           |
//|     Bản riêng M15: không trùng Magic với các khung/hướng khác   |
//+------------------------------------------------------------------+
...
input ulong  MagicNumber      = 203106; // Giữ khác Magic của EA BUY khi chạy cùng symbol
...
//====================================================================
// Comment lệnh cố định M15_SELL — Magic riêng; đối tượng chart prefix GPS_M15_
const string EA_TF_TAG = "M15";
...
void DrawBoxes(double entry, double sl, double tp)
{
   ...
   ObjectCreate(0,"GPS_M15_EntryLine",OBJ_TREND,0,t1,entry,t2,entry);
   ObjectSetInteger(0,"GPS_M15_EntryLine",OBJPROP_COLOR,ColorEntryLine);
   ObjectSetInteger(0,"GPS_M15_EntryLine",OBJPROP_WIDTH,2);
   ObjectSetInteger(0,"GPS_M15_EntryLine",OBJPROP_STYLE,STYLE_SOLID);
   ObjectSetInteger(0,"GPS_M15_EntryLine",OBJPROP_RAY_RIGHT,false);
   ObjectSetInteger(0,"GPS_M15_EntryLine",OBJPROP_SELECTABLE,false);
   ObjectSetInteger(0,"GPS_M15_EntryLine",OBJPROP_HIDDEN,true);

   double slTop=sl, slBot=entry;
   ObjectCreate(0,"GPS_M15_SLBox",OBJ_RECTANGLE,0,t1,slTop,t2,slBot);
   ObjectSetInteger(0,"GPS_M15_SLBox",OBJPROP_COLOR,ColorSLBox);
   ObjectSetInteger(0,"GPS_M15_SLBox",OBJPROP_FILL,true);
   ObjectSetInteger(0,"GPS_M15_SLBox",OBJPROP_BACK,true);
   ObjectSetInteger(0,"GPS_M15_SLBox",OBJPROP_SELECTABLE,false);
   ObjectSetInteger(0,"GPS_M15_SLBox",OBJPROP_HIDDEN,true);

   double tpTop=entry, tpBot=tp;
   ObjectCreate(0,"GPS_M15_TPBox",OBJ_RECTANGLE,0,t1,tpTop,t2,tpBot);
   ObjectSetInteger(0,"GPS_M15_TPBox",OBJPROP_COLOR,ColorTPBox);
   ObjectSetInteger(0,"GPS_M15_TPBox",OBJPROP_FILL,true);
   ObjectSetInteger(0,"GPS_M15_TPBox",OBJPROP_BACK,true);
   ObjectSetInteger(0,"GPS_M15_TPBox",OBJPROP_SELECTABLE,false);
   ObjectSetInteger(0,"GPS_M15_TPBox",OBJPROP_HIDDEN,true);

   string dirTxt="▼ SELL";
   color  dirClr=clrTomato;
   DrawPriceLabel("GPS_M15_LabelDir",dirTxt,t1,entry,dirClr);
   DrawPriceLabel("GPS_M15_LabelEN","Entry: "+DoubleToString(entry,_Digits),t2,entry,ColorEntryLine);
   DrawPriceLabel("GPS_M15_LabelSL","SL: "+DoubleToString(sl,_Digits),t2,sl,ColorSLBox);
   DrawPriceLabel("GPS_M15_LabelTP","TP: "+DoubleToString(tp,_Digits),t2,tp,ColorTPBox);
   ChartRedraw(0);
}
...
{
   datetime t2=iTime(_Symbol,PERIOD_CURRENT,0)+(datetime)PeriodSeconds(PERIOD_CURRENT)*BoxExtendBars;
   string objs[]={"GPS_M15_EntryLine","GPS_M15_SLBox","GPS_M15_TPBox","GPS_M15_LabelEN","GPS_M15_LabelSL","GPS_M15_LabelTP","GPS_M15_LabelDir","GPS_M15_BELine"};
...
void DrawMALines(int barsBack)
{
   ...
   {
      string fn="GPS_M15_FastMA_"+IntegerToString(i);
      string sn="GPS_M15_SlowMA_"+IntegerToString(i);
...
void UpdateSLBox()
{
   ...
   ObjectSetDouble(0,"GPS_M15_SLBox",OBJPROP_PRICE,0,slPrice); ObjectSetDouble(0,"GPS_M15_SLBox",OBJPROP_PRICE,1,slPrice);
   ObjectSetString(0,"GPS_M15_LabelSL",OBJPROP_TEXT,"BE+: "+DoubleToString(slPrice,_Digits));
   ObjectSetInteger(0,"GPS_M15_LabelSL",OBJPROP_COLOR,clrGold);
   ObjectSetDouble(0,"GPS_M15_LabelSL",OBJPROP_PRICE,slPrice);
   datetime t1=iTime(_Symbol,PERIOD_CURRENT,1);
   datetime t2=iTime(_Symbol,PERIOD_CURRENT,0)+(datetime)PeriodSeconds(PERIOD_CURRENT)*BoxExtendBars;
   if(ObjectFind(0,"GPS_M15_BELine")<0) ObjectCreate(0,"GPS_M15_BELine",OBJ_TREND,0,t1,slPrice,t2,slPrice);
   ObjectSetDouble(0,"GPS_M15_BELine",OBJPROP_PRICE,0,slPrice); ObjectSetDouble(0,"GPS_M15_BELine",OBJPROP_PRICE,1,slPrice);
   ObjectSetInteger(0,"GPS_M15_BELine",OBJPROP_COLOR,clrGold); ObjectSetInteger(0,"GPS_M15_BELine",OBJPROP_WIDTH,2);
   ObjectSetInteger(0,"GPS_M15_BELine",OBJPROP_STYLE,STYLE_DASH); ObjectSetInteger(0,"GPS_M15_BELine",OBJPROP_RAY_RIGHT,false);
   ObjectSetInteger(0,"GPS_M15_BELine",OBJPROP_SELECTABLE,false); ObjectSetInteger(0,"GPS_M15_BELine",OBJPROP_HIDDEN,true);
   ChartRedraw(0);
}
...
{
   string name="GPS_M15_Hit_"+IntegerToString(GetTickCount());
...
{
   string obsolete[]={
      "GPS_M15_DIR","GPS_M15_H1","GPS_M15_A1","GPS_M15_A2","GPS_M15_H2","GPS_M15_D0","GPS_M15_D1","GPS_M15_D2","GPS_M15_D3",
      "GPS_M15_H3","GPS_M15_S1","GPS_M15_S2","GPS_M15_H4","GPS_M15_P2","GPS_M15_P3","GPS_M15_P4","GPS_M15_P5",
   ...
   GPS_M15_Label("GPS_M15_H0","TVH-Smile",x,y,clrDeepSkyBlue,12,true); y+=dyTitle;
   GPS_M15_Label("GPS_M15_TS",DashboardSubtitleSell(),x,y,clrSilver,9,false); y+=dySub;
   GPS_M15_Label("GPS_M15_S0","Signal     "+sigText,x,y,sigClr,10,true); y+=dy;
   GPS_M15_Label("GPS_M15_A0","Balance  "+DoubleToString(balance,2)+" "+currency+
      "     ·     Today  "+(totalDayPnL>=0?"+":"")+DoubleToString(totalDayPnL,2)+" "+currency,
      x,y,(totalDayPnL>=0?clrWhite:clrTomato),9,false); y+=dy;

   if(gTrade.active)
   {
      if(ObjectFind(0,"GPS_M15_P0")>=0) ObjectDelete(0,"GPS_M15_P0");
      ...
      GPS_M15_Label("GPS_M15_P1",posLine,x,y,(pnlPts>=0?clrLime:clrTomato),9,false); y+=dy;
   }
   else
   {
      if(ObjectFind(0,"GPS_M15_P1")>=0) ObjectDelete(0,"GPS_M15_P1");
      GPS_M15_Label("GPS_M15_P0","○  No open position",x,y,clrDarkGray,9,false); y+=dy;
   }
   ...
   GPS_M15_Label("GPS_M15_F0",foot,x,y,footClr,8,false);
}

void GPS_M15_Label(string name, string txt, int x, int y, color clr, int sz, bool bold)
{
